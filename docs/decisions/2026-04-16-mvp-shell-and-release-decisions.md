...
- 日期：2026-04-16
- 状态：Accepted
- 关联文档：`docs/plans/2026-04-16-openweave-decision-checklist.md`、`docs/plans/2026-04-16-openweave-mvp-implementation-plan.md`
...
2. Alpha 仅支持 macOS；Windows/Linux 在 PTY 验证完成后再进入预览打包路径。
...
- Windows/Linux 打包不进入当前 MVP 主线，只保留后续扩展路径。
...
3. 在 PTY 验证通过后，再单独评估 Windows MSI 与 Linux 包格式实现路径。
